...
Workbook.actions = {
    ...
    */
    ...
    /**
    * @example
    * {action: WorkbookStream.actions.DECLARE_ROW, type: 'row type', attributes: {ht: 30, customHeight: true}, cells: [{name: 'field1'}, {name: 'field2'}]}
    * @requires type, cells
    */
    DECLARE_ROW: 'DECLARE_ROW',
    /**
    * @example
...
Workbook.prototype.declareRow = function (data, callback) {
    ...
    data.cells.forEach(function (cellData) {
        var cell = new Cell(cellData.name);
    ...
    this.rowTypes[data.rowType] = row;
    callback();
};
Workbook.prototype.addRow = function (data, callback) {
    var ok = this.push(JSON.stringify({
        action: StreamArchivier.actions.WRITE_DATA,
...
Workbook.prototype.addColumn = function (data, callback) {
    var column = new Column(data.min, data.max);
    ...
    this.push(JSON.stringify({
        action: StreamArchivier.actions.WRITE_DATA,
        data: this.activeFile.getColumnXml(column)
    }));
    callback();
};
...
Workbook.prototype._transform = function (chunk, encoding, callback) {
    ...
    switch (chunk.action) {
        ...
        case Workbook.actions.CLOSE_WORKBOOK:
            ...
            break;
        case Workbook.actions.DECLARE_ROW:
